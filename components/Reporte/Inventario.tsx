...
const Inventario = ({width, height, scrollPosition, limite }: any) =>{
    ...
    const fetchProducts = async () => {
        ...
        var newProduct = [{fecha:"", cantidad:"", producto_codigo_interno:"", valor:""}];
        for (var i = 0; i < data.productos.length; i++){

            newProduct.push({fecha: stringToDateToString(data.productos[i].updated_at), 
    ...
    const items = [];
    const [mainProductShown, setMainRecipesShown] = useState([{fecha:"", cantidad:"", producto_codigo_interno:"", valor:""}]);
